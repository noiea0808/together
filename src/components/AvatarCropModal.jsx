...
const OUTPUT = 480   // 업로드용 정사각형 출력 크기(px) — 레티나 화면에서도 흐려 보이지 않도록 여유를 둔다

function clamp(value, min, max) {
  return Math.min(max, Math.max(min, value))
}

// 셀렉트한 원본 사진을 그대로 업로드하지 않고, 원형 미리보기 영역 안에서
// 드래그(위치)·슬라이더(확대)로 보여줄 부분을 고른 뒤 캔버스로 잘라 JPEG로
// 다시 인코딩해서 올린다. HEIC 등 브라우저마다 처리가 갈리는 원본 포맷 문제를
// 우회하는 효과도 있다.
export default function AvatarCropModal({ file, onCancel, onConfirm, uploading }) {
  ...
  return (
    <div style={S.overlay} onClick={() => !uploading && onCancel()}>
      <div style={S.dialog} onClick={e => e.stopPropagation()}>
        <div style={S.title}>사진 위치 조정</div>
