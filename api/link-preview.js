...
//
// youtube.com/shorts/{id}는 서버(특히 Vercel 같은 데이터센터 IP)에서 요청하면 영상별
// og 태그 없이 유튜브 기본 홈 메타만 내려주는 경우가 많다. 같은 영상의 /watch?v={id}
// 페이지는 이 문제 없이 안정적으로 영상별 og 태그를 내려줘서 요청을 그쪽으로 바꿔준다.
function resolveFetchUrl(target) {
  ...
  if (target.hostname === 'www.youtube.com' || target.hostname === 'm.youtube.com' || target.hostname === 'youtube.com') {
    const m = target.pathname.match(/^\/shorts\/([\w-]+)\/?$/)
    if (m) {
      return new URL(`https://www.youtube.com/watch?v=${m[1]}`)
    }
  }
  return target
}
